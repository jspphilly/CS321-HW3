...
public class Util {
     ...
     */
    public static int modExpon(long base, long exponent, long mod){
        //Return immediately if something looks off
        if (base < 1 || exponent < 0 || mod < 1){
           return -1;
        }
        
        //conver the exponenet b to binary value
        String binValue = Long.toBinaryString(exponent);
        long result = -1;
        
        for (int i = 0; i < binValue.length(); i++){
            char c = binValue.charAt(i);        
